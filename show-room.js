const slides = Array.from({length: 20}, (_, i) => ({file:`showroom-${i + 1}.jpg`}));
...
function showEmpty(){image.style.display='none';empty.classList.add('show');}
...
function showSlide(i){
  ...
  setTimeout(()=>{
    ...
    image.alt='Soot Destroyer showroom image';
    progress.style.width=`${((index+1)/slides.length)*100}%`;
    image.classList.remove('fade');
  },160);
  document.querySelectorAll('.thumb').forEach((el,n)=>el.classList.toggle('active',n===index));
  document.querySelectorAll('.dot-button').forEach((el,n)=>el.classList.toggle('active',n===index));
}
...
document.querySelector('#prevSlide').addEventListener('click',prev);
...
stage.addEventListener('touchend',e=>{const delta=e.changedTouches[0].screenX-touchX;if(Math.abs(delta)>45){delta<0?next():prev();}},{passive:true});
stage.addEventListener('mouseenter',()=>clearInterval(timer));
stage.addEventListener('mouseleave',()=>{if(playing)restart();});
buildGallery();
